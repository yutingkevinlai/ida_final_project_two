...
import math
import scipy
project_home = '/home/kevin/caffe_practice/ida_final_project_two/'
...
[h, w, c] = img.shape
img = cv2.resize(img, None, fx=1280/w, fy=960/h, interpolation=cv2.INTER_CUBIC)
...
# Apply KMeans
ret,labels,centers = cv2.kmeans(Z,10,criteria,10,flags)
...
ans3 = np.where(np.logical_and(center[:,0]>160, center[:,0]<256))# 30 80
print ans3
ans4 = np.where(np.logical_and(center[:,1]>113, center[:,1]<200))# 20 150
print ans4
ans_iron = np.intersect1d(ans3,ans4)
ans = np.union1d(ans_socket,ans_iron)
print ans
idx = np.asarray(ans)
print idx
...
count = 0
original_clusters = len(group[0])
for i in range(len(group[0])):
	if int(group[0,count]) < 1:
		...
		count = count - 1
	count = count + 1
processed_clusters = len(group[0])
print '   from ', original_clusters, '=====> ', processed_clusters
...
thresh = 80
original_clusters = processed_clusters
...
for i in range(len(mean[0])):
	...
	#gray_img = tmp_img
	#gray_img = cv2.cvtColor(tmp_img,cv2.COLOR_RGB2GRAY)
